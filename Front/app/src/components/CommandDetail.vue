<template>
  <div class="p-8 max-w-7xl mx-auto">
    ...
    <!-- Content -->
    <div v-else-if="command" class="bg-white/90 dark:bg-gray-800/90 backdrop-blur-md p-8 rounded-2xl shadow-xl border border-white/20">
      
      <!-- Headers -->
      <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-8 flex justify-between items-start">
          <div>
            <h1 class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600 mb-2">{{ command.name }}</h1>
            <p class="text-xl text-gray-600 dark:text-gray-300">{{ command.description }}</p>
          </div>
          <div class="flex items-center space-x-2">
               <span v-if="command.category" class="bg-indigo-100 text-indigo-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-indigo-200 dark:text-indigo-900">{{ command.category }}</span>
          </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-8">
              ...
              <div v-if="command.howTo" class="bg-gray-50 dark:bg-gray-900 rounded-xl p-6 shadow-inner">
               ...
               <div v-if="command.help" class="prose dark:prose-invert max-w-none">
                ...
                <!-- Executer Form -->
               <div class="bg-indigo-50 dark:bg-gray-900 border border-indigo-100 dark:border-gray-700 rounded-xl p-6 mt-8">
          ...
          <div class="lg:col-span-1 space-y-6">
             <div class="bg-gray-50 dark:bg-gray-900 rounded-xl p-6 border border-gray-100 dark:border-gray-700">
                <h3 class="font-bold text-gray-900 dark:text-white mb-4 uppercase text-sm tracking-wider">Permissions</h3>
                ...
                <div class="mb-4">
                    ...
                    <div v-if="command.userPermissions && command.userPermissions.length">
                        <span v-for="perm in command.userPermissions" :key="perm" class="inline-block bg-red-100 text-red-800 text-xs px-2 py-1 rounded mr-1 mb-1">
                            {{ perm }}
                        </span>
                    </div>
                ...
                <div>
                    <span class="text-xs font-semibold text-gray-500 uppercase">Bot</span>
                     <div v-if="command.botPermissions && command.botPermissions.length">
                        <span v-for="perm in command.botPermissions" :key="perm" class="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded mr-1 mb-1">
                            {{ perm }}
                        </span>
                    </div>
                    <div v-else class="text-sm text-gray-400 italic">None</div>
      ...
      <div class="mt-12 pt-6 border-t border-gray-200 dark:border-gray-700">
        <router-link to="/commands" class="inline-flex items-center text-indigo-600 hover:text-indigo-800 font-medium transition-colors">
...
export default {
  ...
  computed: {
    ...mapState(useMainStore, ['commands']),
  },
  methods: {
    ...
    async executeCommand() {
        ...
        try {
            ...
            alert('Command executed successfully!\nCheck bot logs or channel for output.');
            
        } catch (error) {
            console.error(error);
            alert('Failed to execute command: ' + (error.response?.data?.message || error.message));
        } finally {
